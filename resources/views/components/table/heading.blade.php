...
<th {{$attributes->merge(['class' => 'px-6 py-4 font-medium text-gray-900'])->only('class')}}>
    ...
    @else
        <button {{$attributes->merge()->except('class')}} class="flex items-center space-x-1 text-left text-xs leading-4 font-medium text-cool-gray-500 uppercase tracking-wider group focus:outline-none focus:underline">
            <span>
                {{$slot}}
            </span>
